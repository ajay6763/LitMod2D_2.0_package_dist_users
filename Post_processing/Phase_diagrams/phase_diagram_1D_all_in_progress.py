...
l1_Ph= l1_Ph+ tuple(np.zeros(len(l1_Cpx)-len(l1_Ph)))
...
l1_Sp= l1_Sp+ tuple(np.zeros(len(l1_Ph)-len(l1_Sp)))
print len(l2_Sp),len(l2_Ph),len(l2_Cpx)

#####################3
########## Put all together
mineral=np.column_stack((l2_Ol,l2_Gt,l2_Opx,l2_Cpx,l2_C2c,l2_Ph,l2_Sp))
...
ax1.fill_between(l1_C2c,cum_x_Ph,cum_x_Cpx,label='Plagioclase',color='orange',alpha=0.4)


########################
### Spinel
#cum_x_Sp=[x + y for x, y in zip(cum_x_Ph, l2_Sp)]
cum_x_Sp=mineral[:,0]+mineral[:,1]+mineral[:,2]+mineral[:,3]+mineral[:,4]+mineral[:,5]+mineral[:,6]
ax1.plot(l1_Ph,cum_x_Sp,color='black')#label='Spinel')#,linestyle='--',color='black',label='500km Anomaly')
ax1.fill_between(l1_Ph,cum_x_Sp,cum_x_Ph,label='Spinel',color='red')
...
plt.legend( fancybox=True,shadow=False, facecolor='grey',framealpha=1,loc='upper left',fontsize=legend_font_size)
