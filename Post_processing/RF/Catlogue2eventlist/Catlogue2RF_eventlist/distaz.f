c----------------------------- PROGRAM DIST ----------------------------
c
c   Computes the distance and azimuth between two points. Program
c   requests the coordinates (in decimal degrees) of two points, A
c   and B.  The distance and azimuth are returned, and a new point B is
c   requested. c   Terminate the program with a <cr>.
c
c-----------------------------------------------------------------------
      character input*127
      open(99,file='nul.tmp')
c
      write(*,'(/a)') ' Coordinates of point A (lat long): '
      read(*,'(a)') input
      if (input .eq. ' ') stop ' '
      read(input,*) alat, alon
10    write(*,'(a)') ' Coordinates of point B(lat long): '
      read(*,'(a)') input
      if (input .eq. ' ') stop ' '
      read(input,*) blat, blon
      call distaz(alat,alon,blat,blon,azm,bzm,ddg,dkm)
      write(*,15) dkm, ddg, azm, bzm
15    format(' Dist(km)=',f8.2,3x,'Dist(dg)=',f5.1,3x,
     &	      'Azimuth=',f5.1,3x,'Back Azimuth=',f5.1,/)
      goto 10
c
      end
